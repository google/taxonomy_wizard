...
-- INSTRUCTIONS ---------------------------------------------
1. Go to the "Cloud Config" tab of the Admin sheet copied at the end of Deployment.
2. Copy the Project Id to \"Taxonomy Data: Cloud Project Id\" ('Cloud Config'!B1).
3. Copy the the Configurator Cloud Function Endpoint to \"Cloud Function Endpoint\" ('Cloud Config'!B2).
   (You should not need to update the Taxonomy Data BigQuery Dataset.)
4. Open Apps Script from the Google Sheet (Extensions→Apps Script).
5. On the LHR, click on Project Settings (the gear icon: ⚙ ).
6. In the \"Google Cloud Platform (GCP) Project\" section, click on \"Change Project\".
